...
```
   ...
   加载段选择子`8`到CS寄存器后，从GDT中取出该段的描述符，也就是代码段，读取它的基地址和段长。
```
.word   0x07FF       ! 段限长（8 MB）
.word   0x0000       ! 段基址低 16 位
.word   0x9A00       ! 类型：代码段，可读/执行
.word   0x00C0       ! 粒度（4 KB 页），32 位模式
```
   然后在代码段的基础上加上偏移量`0`，因此EIP为0。所以最终的寻址结果就是`CS:EIP`为`0x08:0x00`。
